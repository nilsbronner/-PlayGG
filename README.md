...
La première crée les tables `signatures` et `confirmation_tokens` avec RLS
activé et sans policy publique — toutes les opérations passent par la clé
`service_role`, jamais exposée au navigateur. La seconde ajoute la colonne
`quality`.
